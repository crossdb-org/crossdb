...
static xdb_objm_t	s_xdb_sub_list;
...
XDB_STATIC xdb_sub_t* 
xdb_find_sub (const char *sub_name)
{
	return xdb_objm_get (&s_xdb_sub_list, sub_name);
}
...
xdb_run_sub (void *pArg)
{
	...
	while (1) {
		pPubConn = xdb_connect (pSub->pub_host, NULL, NULL, NULL, pSub->pub_port);
		if (NULL != pPubConn) {
			break;
		}
		sleep (2);
	...
	pRes = xdb_pexec (pPubConn, "SUBSCRIBE %s CLIENT='%s', CREATE=1, TABLES='%s'", pSub->pub_name, XDB_OBJ_NAME(pSub), pSub->tables);
	XDB_RESCHK(pRes);
	while (1) {
		int len;
...
XDB_STATIC int 
xdb_create_sub (xdb_stmt_sub_t *pStmt)
{
	xdb_conn_t* pConn = pStmt->pConn;
	xdb_sub_t *pSub = xdb_find_sub (pStmt->sub_name);
	if (pStmt->bIfExistOrNot && (NULL != pSub)) {
		return XDB_OK;
	}

	pSub = xdb_calloc (sizeof(xdb_sub_t));
	XDB_EXPECT (NULL != pSub, XDB_E_MEMORY, "Can't alloc memory");
	xdb_strcpy (XDB_OBJ_NAME(pSub), pStmt->sub_name);

	pSub->pConn = pConn;
	xdb_strcpy (pSub->pub_name, pStmt->pub_name);
	xdb_strcpy (pSub->pub_host, pStmt->pub_host);
	pSub->pub_port = pStmt->pub_port;
	if (NULL != pStmt->tables) {
		pSub->tables = xdb_strdup (pStmt->tables, 0);
		XDB_EXPECT (NULL != pSub, XDB_E_MEMORY, "Can't alloc memory");
	}

	XDB_OBJ_ID(pSub) = -1;
	xdb_objm_add (&s_xdb_sub_list, pSub);

	xdb_create_thread (&pSub->tid, NULL, xdb_run_sub, pSub);
...
xdb_initial_sync (xdb_subscribe_t *pSubscribe)
{
	if (!pSubscribe->bSync) {
		...
		for (int i = 0; i < pTbls->count; ++i) {
			...
			while (NULL != (pRow = xdb_fetch_row (pRes))) {
				...
				xdb_subscribe_send (pSubscribe->pConn->pSubscribe, sql_buf, slen);
			}					
	...
	if (pSubscribe->cud_cache.count) {
		xdb_subscribe_send (pSubscribe->pConn->pSubscribe, (const char*)pSubscribe->cud_cache.pBuf, pSubscribe->cud_cache.count);
...
{
	xdb_conn_t* pConn = pStmt->pConn;
	xdb_subscribe_t *pSubscribe = NULL;
	xdb_buf_t	dbuf;
	xdb_buf_init (&dbuf);
	...
	pSubscribe->bCreate = pStmt->bCreate;
	...
	if (pPub != NULL) {
		...
		xdb_pubsublog ("want %s\n", pStmt->tables?pStmt->tables:"");
		if (NULL != pStmt->tables) {
