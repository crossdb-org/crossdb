...
UTEST_I(XdbTestRows, agg_minmax, 2)
{
	...
	CHECK_EXP(pRes, 1, ASSERT_EQ(*(int*)pRow[0], 1000));

	pRes = xdb_exec (pConn, "SELECT MAX(id) FROM student");
	CHECK_EXP(pRes, 1, ASSERT_EQ(*(int*)pRow[0], 1006));

	pRes = xdb_exec (pConn, "SELECT MIN(id),MAX(id) FROM student");
	CHECK_EXP(pRes, 1, ASSERT_EQ(*(int*)pRow[0], 1000); ASSERT_EQ(*(int*)pRow[1], 1006));

	pRes = xdb_exec (pConn, "SELECT MIN(id),MAX(id) FROM student WHERE id > 1001");
	CHECK_EXP(pRes, 1, ASSERT_EQ(*(int*)pRow[0], 1002); ASSERT_EQ(*(int*)pRow[1], 1006));
}
...
UTEST_I(XdbTestRows, agg_sumavg, 2)
{
	...
	CHECK_EXP(pRes, 1, ASSERT_EQ(*(int64_t*)pRow[0], sum));

	pRes = xdb_exec (pConn, "SELECT AVG(id) FROM student");
	CHECK_EXP(pRes, 1, ASSERT_NEAR(*(double*)pRow[0], (double)sum/7, 0.000001));

	pRes = xdb_exec (pConn, "SELECT SUM(id),AVG(id) FROM student");
	CHECK_EXP(pRes, 1, ASSERT_EQ(*(int64_t*)pRow[0], sum); ASSERT_NEAR(*(double*)pRow[1], (double)sum/7, 0.000001));

	pRes = xdb_exec (pConn, "SELECT SUM(id),AVG(id) FROM student WHERE id > 1001");
	sum -= (1000 + 1001);
	CHECK_EXP(pRes, 1, ASSERT_EQ(*(int64_t*)pRow[0], sum); ASSERT_NEAR(*(double*)pRow[1], (double)sum/5, 0.000001));
}
